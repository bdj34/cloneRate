...
ggplot(mal_vs_norm.df, aes(x = malNorm, y = meanEstimate)) +
  geom_label(label = paste0("p = ", round(nonparamTest$p.value, 3)), x = 1.2, y = 1.2) +
...
ggplot(PD9478_long, aes(x = Age, y = VAF)) +
  ...
  geom_line(data = predict.df, aes(x = x, y = y), color = fitColor, size = 1, show.legend = T) +
