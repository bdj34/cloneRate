...
# Stratify by mean net growth rate
corCheck <- cor.test(malMax$estimate, malMax$latency_to_dx)
print(corCheck)
print(corCheck$estimate^2)

# Let's plot the growth rate and the age estimate, and the mean of the maximum growth rate
ggplot(malMax) + geom_point(aes(x = estimate, y = latency_to_dx)) + theme_bw() +
  geom_vline(xintercept = mean(malMax$estimate), linetype = "dashed")

# Stratify the population by the mean growth rate and see if time to diagnosis is signficantly different
malMax$aboveMean <- malMax$estimate > mean(malMax$estimate)

# Because there is no censored data, we can just compare the times using the wilcox test
wilcox.test(latency_to_dx ~ aboveMean, data = malMax)
```
...
# If you have the survival package, compare this to the result using these functions
malMax$status <- 1
...
# If the ggsurvfit package is present, we
survfit2(Surv(time = latency_to_dx, event = status) ~ aboveMean, data = malMax) %>%
  ...
  )+scale_color_manual(values = c(colors[4], colors[2]))+
  theme(axis.text = element_text(size = 16),
        axis.title = element_text(size = 16),
        legend.position = "none") + 
  ggplot2::geom_label(label=paste0("p = ", round(survivalTest$pvalue, 4)), x = 45, y = .85, color = "black", fill = "white")
```
...
# Determine time from clone initiation to diagnosis
malYoungest$latency_to_dx <- malYoungest$cloneAgeEstimate - (malYoungest$age - malYoungest$diagnosis.age)

# Stratify by mean net growth rate
corCheck <- cor.test(malYoungest$estimate, malYoungest$latency_to_dx)
print(corCheck)
print(corCheck$estimate^2)

# Let's plot the growth rate and the age estimate, and the mean of the maximum growth rate
ggplot(malYoungest) + geom_point(aes(x = estimate, y = latency_to_dx)) + theme_bw() +
  geom_vline(xintercept = mean(malYoungest$estimate), linetype = "dashed")

# Stratify the population by the mean growth rate and see if time to diagnosis is signficantly different
malYoungest$aboveMean <- malYoungest$estimate > mean(malYoungest$estimate)

# Because there is no censored data, we can just compare the times using the wilcox test
wilcox.test(latency_to_dx ~ aboveMean, data = malYoungest)

# If you have the survival, compare this to the result using these functions
if(requireNamespace("survival")){
  malYoungest$status <- 1
  survivalTest <- survival::survdiff(survival::Surv(latency_to_dx, status) ~ aboveMean, data = malYoungest)
  print(survivalTest$pvalue)
}
