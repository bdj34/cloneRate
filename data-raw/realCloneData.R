...
cloneDriver <- "unknown"
cloneName <- paste0(ID, "clone1") # Not labelled in paper but label from left to right from figure
...
for (i in 1:length(williams_raw)) {
  ...
  for (j in (1:length(unique_ages))) {
    ...
    separate_object$metadata <- data.frame(
...
cloneName <- paste0(ID, "clone2") # Don't match the other clone name because they aren't the same
cloneTree <- keep.tip(tree, cloneTips)
...
cloneDriver <- "DNMT3A:p.R882H_AND_JAK2:p.V617F"
cloneName <- paste0(ID, "clone2") # Match naming with PD6629_1 (first timepoint)
cloneTree <- keep.tip(tree, cloneTips)
...
cloneDriver <- "DNMT3A:p.?_AND_JAK2:p.V617F"
cloneName <- paste0(ID, "clone2") # Match clone name from previous timepoint
cloneTree <- keep.tip(tree, cloneTips)
...
cloneDriver <- "JAK2:p.V617F_AND_9pUPD_AND_TET2:p.Q1632*_AND_7p-_AND_7q+"
cloneName <- paste0(ID, "clone1") # Match PD4781_1 naming
cloneTree <- keep.tip(tree, cloneTips)
