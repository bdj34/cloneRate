...
All of these can be installed from CRAN as shown below:
...
We require that a clone must have at least `n=10` tips (cells)  to be 
...
```{r plotSubclone, fig.asp = 0.8, fig.width = 7}
...
Not bad! With a correlation coefficient around 0.98, the estimates from each method agree very well.

## Understanding the function outputs

Each of the functions ([maxLikelihood()] and [internalLengths()]) returns a data.frame with
...
`base::cbind()` because we know the order of the metadata is the same as the 
...
estimate performs the best.

## Recreating figures

Here, we recreate the analysis in [Figure 5E-G of our work](https://www.biorxiv.org/).
...
All in all, not too bad! For now, we'll leave the others for you to explore. This vignette has already been a bit long. But now you should have the tools to reproduce our analysis relating to the real data aspect of [our paper](https://www.biorxiv.org/). Hopefully you were able to do this in minutes and hours, which is much better than the months it took me!
