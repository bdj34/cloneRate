...
recent preprint](https://www.biorxiv.org/) analyzing single-cell Whole Genome Sequencing 
...
using the `ape` package function `ape::plot.phylo()` along with some other `ape` functions.
If you have `cloneRate` installed, you already have `ape`. We'll also be using
`ggplot2` to make our plots, and you can install from CRAN as shown below:
...
Unfortunately, we don't have a ground truth to compare to when working with real data. However, this individual clone has longitudinal sequencing data, which we can use to estimate an orthogonal growth rate. We'll come back to this at the end of the vignette.

First, let's apply our analysis to all of the clone trees. If we give a list of `phylo` objects, our growth rate functions will estimate the growth rate of all of them separately, returning a dataframe with each row telling us the result from each tree.
...
Not bad! Each of the functions (maxLikelihood() and internalLengths()) returns a data.frame with the same columns, and we'll walk through what the columns mean here:
...
* `runtime_s` is the runtime of our method in seconds.
* `method` is the method used, which helps if we want to combine data.frames output from different methods for plotting.
* `names` is a unique name for the clone corresponding to the individual ID and clone number. For individuals from the [Williams paper](https://pubmed.ncbi.nlm.nih.gov/35058638/), where some individuals were sampled at multiple timepoints, the ID will be followed by "_1", "_2", or "_3" to indicate the timepoint.
...
# Preview and make sure everything looks right
print(head(metadataAll))
```

Let's join the data.frames containing the metadata and the results. We can use cbind from base R because we know the order of the metadata is the same as the order of the results data.frame. We can check this by comparing the `cloneName_result` and `cloneName_meta` columns, which should be (and are) the same.
...
We'll go forward with our analysis using the results data.frame, which contains the estimates from the maxLikelihood() function. In the [simulation vignette](cloneRate-simulate.html), we show that the maximum likelihood estimate performs the best.
...
If we split the cohort by MPN and normal, we might expect differences in the growth rates. Because the number of sampled cells, on average, is higher in the healthy individuals due to study design, we're more likely to pick up more clones from the healthy individuals. To avoid this bias as much as possible, we'll compare the mean growth rate between individuals with and without MPN.
...
ggplot(uniqueResults, aes(x = multiDriver, y = estimate)) +
  ...
  scale_color_manual(values = c(colors[4], colors[8])) +
  theme_bw() +
...
## Time from clone initiation to diagnosis ([Fig. 5G](https://www.biorxiv.org/))
...
We see that in 15/19 cases, the youngest clone (with >= 10 tips)  is also the most fit. First, let's use the most fit clones and stratify by the mean, reproducing [our result in Figure 5G.](https://www.biorxiv.org/)
...
survfit2(Surv(time = latency_to_dx, event = status) ~ aboveMean, data = malMax) %>%
  ...
  ) + scale_color_manual(values = c(colors[7], colors[3])) +
  theme(legend.position = "none") +
  geom_label(label = paste0("p = ", round(survivalTest$pvalue, 4)), x = 45, y = .85, color = "black", fill = "white") +
  geom_label(label = "r < mean", x = 40, y = .375, color = colors[7], fill = "white") +
  geom_label(label = "r > mean", x = 12, y = .35, color = colors[3], fill = "white")
```
...
As it turns out, the youngest clone is the fittest clone in all 13 individuals diagnosed with MPN. We saw before that this was the case in 15 of the 19 individuals when including MPN and healthy. 
...
# From before, we can find which clone IDs appear twice
tmp <- table(results$uniqueCloneID)
repeatsVec <- names(tmp)[tmp == 2]
...
ggplot(repeatResults) +
  ...
  scale_color_manual(values = colors[c(1, 6, 7)]) +
  theme_bw() +
...
Let's now fit a logistic curve to the longitudinal data and estimate the growth rate for one of these clones, from individual PD9478, which is the tree we showed at the beginning of this vignette. Here, we are recreating some of the analysis from [our Figure 6.](https://www.biorxiv.org/)
...
fit <- nls(VAF ~ K / (1 + exp(-(phi2 + r * Age))),
  ...
  data = PD9478_long, trace = F, algorithm = "port", lower = c(0, -500, 0.00001), upper = c(0.5, 0, 5)
)
...
ggplot(PD9478_long, aes(x = Age, y = VAF)) +
  ...
  geom_line(data = predict.df, aes(x = x, y = y), color = fitColor, linewidth = 1, show.legend = T) +
  geom_point(color = "#808080", shape = 18) +
...
All in all, not too bad! For now, we'll leave the others for you to explore. This vignette has already been a bit long. But now you should have the tools to reproduce all of our analyses relating to the real data aspect of [our paper](https://www.biorxiv.org/). Hopefully you were able to do this in minutes and hours, which is much better than the months it took me!
...
And here's a final link to [our paper](https://www.biorxiv.org/) for more of the details of the methods and data analysis.
