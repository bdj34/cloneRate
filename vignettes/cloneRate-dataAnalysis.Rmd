...
First, let's apply our analysis to all of the clone trees!
...
ggplot(mal_vs_norm.df, aes(x = malNorm, y = meanEstimate)) + 
  ...
  theme_bw()+ ylab("Mean net growth rate (r)") + xlab("") + 
...
ggplot(uniqueResults, aes(x = multiDriver, y = estimate)) + 
  ...
  theme_bw()+ geom_label(label = paste0("p=", round(nonParamTest$p.value, 7)),x = 1.5, y = 2)+
  ylab("Net growth rate (r)")+xlab("")+ ggtitle("Driver mutations")+
...
ggplot(repeatResults) + 
  geom_pointrange(data = repeatResults, aes(x = cloneName_result, y = estimate, ymin = lowerBound, 
                      ymax = upperBound, color = uniqueCloneID))+
  scale_color_manual(values = colors[c(1, 6, 7)]) + theme_bw()+ ylab("Net growth rate estimate (r)")+
...
combine.df <- data.frame("Clone" = "PD9478_1_clone1",
...
ggplot(combine.df) + geom_pointrange(aes(x = method, y = r, ymin = lowerBound, ymax = upperBound)) +
  theme_bw() + ylab("Net growth rate (r)")
