...
We know that our actual growth rate is 1, so we can evaluate how our estimates are doing. One estimate doesn't really tell us much though. Let's try 100. To do this, we just set the `nTrees` param equal to 100 in our `simUltra()` function. You can parallelize this process if you have the `parallel` package installed by simply setting `nCores`, but we'll leave this at 1 for now.
...
Alternatively, we can generate a new set of mutation trees using the function `simMut()`:

```{r simMut}
# Set params for ultra tree + a mutation rate
mutTree.list2 <- simMut(a = 1, b = 0, cloneAge = 20, n = 100, nTrees = 100, nu = 100)
```
...
## Max likelihood vs. internal lengths

We want to know which estimate is the best. Let's simulate a whole bunch of trees and compare, 1000 should be enough. Note that simulating a thousand trees, while waayyyy quicker than doing 1000 gillespie simulations, still takes some time and can be parallelized by setting the `nCores` param. We won't do this because it's not allowed for CRAN vignettes.

Let's keep the net growth rate, `r` at 1, to make comparing estimates easy. However, we can vary `a` (and, subsequently `b`).

```{r drawa}
# Draw a (birth rate), then set b (death rate), so that net growth rate is 1
a_vec <- stats::runif(500, min = 1, max = 3)
b_vec <- a_vec - 1
```

We'll keep the clone age at 20 years and the number of samples at `n=100` for now. We'll discuss the impact of changing the growth rate and/or time in [Varying r], and the impact of changing the number of samples in [Varying n]. Note that we have to explicitly set the `nTrees` parameter to be equal to the number of trees we want, which is 500 and equal to the length of the `a_vec` we just created.

```{r sim500}
# Generate 500 trees and see how long it takes
ptm <- proc.time()
manyTrees <- simUltra(a = a_vec, b = b_vec, cloneAge = 20, n = 100, nTrees = length(a_vec), nCores = 1)
print(proc.time()[["elapsed"]] - ptm[["elapsed"]])
```

Now, we can simply apply the ultrametric growth rate estimates

```{r applyThousand}
# Estimate growth rate using maximum likelihood
resultsML_500 <- maxLikelihood(manyTrees)

# Estimate growth rate using internal lengths
resultsLengths_500 <- internalLengths(manyTrees)
```

Let's plot just like above

```{r plotThousand}
combine500 <- rbind(resultsML_500, resultsLengths_500)
...
ggplot(combine500) +
  ...
  theme_bw() + theme(axis.text.y = element_blank(), axis.ticks.y = element_blank(),
                     legend.title = element_blank())+
  xlab("Net growth rate estimate (r)") + ylab("Density")+ggtitle("Estimates from 500 simulated trees")
  scale_color_manual(labels = c("Internal lengths", "Max. likelihood"), values = c("black", "#009E73"))
```
...
groundTruth <- 1
rmse <- unlist(lapply(
  split(combine1000, combine1000$method),
  function(x) {
...
```{r simRuntime}
# Calculate the runtime for making the trees
treeSimRuntime <- unlist(lapply(
  thousandTrees,
  function(x) {
    x$metadata$runtime
  }
